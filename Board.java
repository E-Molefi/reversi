...
public class Board
{
     ...
     */
    public String getSkippedPlayer()
    {
            ...
            for(int j = 0; j < getBoardSize(); j++) {
                if(source == discs[i][j]) {
                    ...
                    if(canMove(getCurrentPlayer(), i, j)) {
                        ...
                        illegalMove = false;
                        // Play the position
                        discs[i][j].setText(getCurrentPlayer());
    ...
    {
        if(getCurrentPlayer().equals(BLACK_PLAYER)) {
     ...
     * @param
     */
    private void capture(String player, int verticalPosition, int horizontalPosition)
    {
        String enemy = "";
        String friend = "";
        ...
        for(int i = 0; i < boardDirections.length; i++) {
            ...
            if((horizontalPosition+yDirection) > -1 && (horizontalPosition+yDirection) < getBoardSize() && (verticalPosition+xDirection) < getBoardSize() && (verticalPosition+xDirection) > -1 ) {
                ...
                if(discs[verticalPosition+xDirection][horizontalPosition+yDirection].getText().equals(enemy)) {
            ...
            if(potential) {
                ...
                && (verticalPosition+(step*xDirection)) < getBoardSize() && (verticalPosition+(step*xDirection)) > -1 ) {
                    ...
                    if(discs[verticalPosition+(step*xDirection)][horizontalPosition+(step*yDirection)].getText().equals("")) {                        
                        break;
                    }

                    if(discs[verticalPosition+(step*xDirection)][horizontalPosition+(step*yDirection)].getText().equals(friend)) {

                        for(int z = 1; z < step; z++) {
                            discs[verticalPosition + (z * xDirection)][horizontalPosition + (z * yDirection)].setText(friend);
                        }
                        break;
                    }                    
                    step++;
     ...
     * @param
     */
    public boolean isMoveLegal(String player, int verticalPosition, int horizontalPosition)
    {
        ...
        if((discs[verticalPosition][horizontalPosition].getText().equals(""))) {
     ...
     *               Returns false otherwise.
     */
    public boolean canMove(String player, int x,int y)
    {
     ...
     */
    public int validMoves(String player)
    {
        int counter = 0;
     ...
     */
    public void displayValidMoves(String player)
    {
        for(int x = 0; x < discs.length; x++) {
            for(int y = 0; y < discs.length; y++) {
                // Set up the coords for testing
    ...
    public void score()
    {
        ...
        for(int x = 0; x < getBoardSize(); x++) {
            for(int y = 0; y < getBoardSize(); y++) {
                if(discs[x][y].getText().equals(BLACK_PLAYER)) {
                    scoreBlack++;
                }

                if(discs[x][y].getText().equals(WHITE_PLAYER)) {
                    scoreWhite++;
                }
            }
        }
